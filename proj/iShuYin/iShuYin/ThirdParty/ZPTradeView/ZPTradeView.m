...
- (void)removeTradeViewFromSuperView
{
    [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            self.inputView.top = -_inputView.height;
            self.cover.alpha = 0.01;
        } completion:^(BOOL finished) {
...
- (void)showInView:(UIView *)view
{
    ...
    [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
//        self.inputView.y = (self.height - self.inputView.height - _keyboardHeight) * 0.5 + 1;
        self.inputView.top = 100;
        self.cover.alpha = 0.01;
    } completion:^(BOOL finished) {
        [UIView animateWithDuration:0.1 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            self.inputView.top -= 1;
        }completion:^(BOOL finished) {
            [UIView animateWithDuration:0.05 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                self.inputView.top += 10;
            }completion:^(BOOL finished) {
                [UIView animateWithDuration:0.05 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                    self.inputView.top -= 5;
                }completion:^(BOOL finished) {
